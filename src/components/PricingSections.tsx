...
import { Button, Card, SectionWrapper } from './UI';
...
export const PricingRecap: React.FC<{
  ...
  return (
    <SectionWrapper id="value_stack" background="#7f8b79" className="text-white">
      <div className="max-w-3xl mx-auto">
        <h2 className="text-2xl md:text-3xl text-center mb-12 text-white whitespace-pre-line">{title}</h2>
        <div className="bg-white/10 rounded-card border border-white/20 overflow-hidden mb-8">
          {priceItems.map((item, i) => (
            <div key={i} className={`flex justify-between items-center p-4 md:p-6 ${i !== priceItems.length - 1 ? 'border-bottom border-white/10' : ''}`}>
...
export const OfferSplit: React.FC<{
  ...
  return (
    <SectionWrapper id="offer_cta" className="py-16 md:py-24">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        <div className="flex justify-center">
          <img 
            src={leftImage} 
            alt="Mockup" 
            className="w-full max-w-md h-auto drop-shadow-[0_20px_50px_rgba(0,0,0,0.2)]"
        ...
        <div className="text-center lg:text-left">
          ...
          <div className="space-y-3 mb-10">
            {supportingLines.map((line, i) => (
              <div key={i} className="flex items-center justify-center lg:justify-start gap-2 text-olive font-bold">
...
export const ThreeCards: React.FC<{
  ...
  return (
    <SectionWrapper background="#7f8b79" className="text-white">
      <h2 className="text-2xl md:text-3xl text-center mb-16 text-white">{title}</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {cards.map((card, i) => (
          <div key={i} className="bg-white/10 p-8 rounded-card border border-white/20 text-center flex flex-col items-center">
